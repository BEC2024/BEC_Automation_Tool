...
Public Class CommonReportExcelUtil
    ...
    Public Sub checkExistFiles(ByVal comObj As CommonReport)
        Try
            ...
            If System.IO.File.Exists(strFileName) Then
                ...
                For i = 2 To str2
                    If wSheet.Cells(i, 1).value = Nothing Then  'And wSheet.Cells(i, 2).value = Nothing  '28th Oct 2024
                        Exit For
                    End If
    ...
    Public Sub NewExcelReport(ByVal comObj As CommonReport)
        ...
        Try
            ...
            worksheet.Name = "KPI Report"

            worksheet.Cells.Validation.Delete()
            worksheet.Cells.NumberFormat = "@"

            For colIndex As Integer = 0 To dt.Columns.Count - 1
                worksheet.Cells(1, colIndex + 1).Value = dt.Columns(colIndex).ColumnName
            Next
            ...
            For rowIndex As Integer = 0 To dt.Rows.Count - 1
                ...
                For colIndex As Integer = 0 To dt.Columns.Count - 1
                    worksheet.Cells(RowCount, colIndex + 1).Value = row(colIndex).ToString()
                Next
            ...
            Dim filePath As String = Path.Combine(comObj.dir, "KPI_Report.xlsx")
        ...
        Catch ex As Exception
            MsgBox($"Error while creating New KPI Report in Excel: {vbNewLine}{ex.Message}{vbNewLine}{ex.StackTrace}")
        End Try
    End Sub
    ...
    Public Sub NewCSV_Report(ByVal comObj As CommonReport)
        Try
            ...
            For Each col As DataColumn In dt.Columns
                builder.Append(col.ColumnName).Append(sepChar)
                'sep = sepChar
            Next

            writer.WriteLine(builder.ToString().TrimEnd(sepChar))
            builder.Clear()
            ...
            '        Dim FillValueInCell As String = $"'{value}"
            '        builder.Append(FillValueInCell).Append(",")
            '        'sep = sepChar
            '    Next
            ...
            For Each row As DataRow In dt.Rows
                For i As Integer = 0 To dt.Columns.Count - 1
                    ...
                    ' Format the second column as text
                    If i = 1 Then
                        Dim FillValueInCell As String = $"{value}"
                        builder.Append("""").Append(FillValueInCell).Append("""")
                    Else
                        builder.Append(value)
                    End If

                    ' Append separator if not the last column
    ...
    Public Sub newKPI_Report(ByVal comObj As CommonReport)
        Try
            ...
            '8th Nov 2024
            wSheet2.Cells.Validation.Delete()
            wSheet2.Cells.NumberFormat = "@"

            '28th Oct 2024         
            comObj.MTRfiles = New ArrayList()
            ...
            wSheet2.Range("A1").Columns.EntireColumn.AutoFit()
            ...
            For i = 2 To str3
                ...
                ''Added by vimal 6 nov 2024
                'formatRange.Cells.NumberFormat = "@"
        ...
        Try
            ...
            For Each dr In dt2.Rows
                ...
                For Each dc In dt2.Columns
                    ...
                    '8th Nov 2024
                    _excel.Cells(rowIndex + 1, colIndex).Cells.Validation.Delete()
                    _excel.Cells(rowIndex + 1, colIndex).Cells.NumberFormat = "@"
            ...
            wSheet.Range("A1").Columns.EntireColumn.AutoFit()
            formatRange.Font.Name = "Arial"
            formatRange.Font.Size = 11
        ...
        Try
            ...
            For Each row As DataRow In dt.Rows
                For i As Integer = 0 To dt.Columns.Count - 1
                    ...
                    ' Check if it's the second column, then format as text
                    If i = 1 Then
                        builder.Append("""").Append(value).Append("""") ' Add quotes for the second column
                    Else
                        builder.Append(value)
                    End If
                    If i < dt.Columns.Count - 1 Then
                        builder.Append(",") ' Add comma between columns
                    End If
                Next
                writer.WriteLine(builder.ToString())
                builder.Clear()
...
#End Region
            ...
            For i = 4 To ColCnt
                ...
                '1PartName Value(Error)
                comObj.partName1 = If(xlsSheet1.Rows(2).Cells(i).value = Nothing, er, xlsSheet1.Rows(2).Cells(i).value.ToString())

                '2Category
                comObj.category2 = "Assembly"

                '3author name/Who is the Author of the file?
                comObj.AuthorName3 = If(xlsSheet1.Rows(6).Cells(i).value = Nothing, er, xlsSheet1.Rows(6).Cells(i).value.ToString())
                ...
                '5part number match value/Is the part number match with M2M? *
                comObj.partNumberMatch5 = If(xlsSheet1.Rows(4).Cells(i).value = Nothing, er, xlsSheet1.Rows(4).Cells(i).value.ToString())

                '6document number/Is the Document Number correct? (BEC designed parts must have proper doc number.Hardware And other exceptions are N/A) *
                comObj.documentNumber6 = If(xlsSheet1.Rows(9).Cells(i).value = Nothing, er, xlsSheet1.Rows(9).Cells(i).value.ToString())

                '7authorNameIs the "Author" field populated with the full name? (DGS for DGSTS models regardless of individual name) 
                comObj.AuthorName7 = If(xlsSheet1.Rows(10).Cells(i).value = Nothing, er, xlsSheet1.Rows(10).Cells(i).value.ToString())

                '8dashinunusedfield/Do all technically unused properties have a "dash" populated? 
                comObj.DashInUnusedFiled8 = If(xlsSheet1.Rows(11).Cells(i).value = Nothing, er, xlsSheet1.Rows(11).Cells(i).value.ToString())

                '9 m2mdescription/Does the Model Title MATCH the M2M Item Master (INV) Description Field?
                comObj.m2mDiscriptionMatch9 = If(xlsSheet1.Rows(12).Cells(i).value = Nothing, er, xlsSheet1.Rows(12).Cells(i).value.ToString())

                '10uomsmatch/Do all UOMs match M2M? (i.e. EA, sqft, in, etc.)
                comObj.uomsMatch10 = If(xlsSheet1.Rows(13).Cells(i).value = Nothing, er, xlsSheet1.Rows(13).Cells(i).value.ToString())

                '11interface/Is interfernces found in assembly?
                comObj.interface11 = If(xlsSheet1.Rows(15).Cells(i).value = Nothing, er, xlsSheet1.Rows(15).Cells(i).value.ToString())

                '12interpartcopiesIs interfernces found in assembly?
                comObj.interpartcopies12 = If(xlsSheet1.Rows(16).Cells(i).value = Nothing, er, xlsSheet1.Rows(16).Cells(i).value.ToString())

                '13partcopiespart copies detected
                comObj.partcopies13 = If(xlsSheet1.Rows(17).Cells(i).value = Nothing, er, xlsSheet1.Rows(17).Cells(i).value.ToString())

                '14brokenfilebroken  file Path detected
                comObj.brokefile14 = If(xlsSheet1.Rows(18).Cells(i).value = Nothing, er, xlsSheet1.Rows(18).Cells(i).value.ToString())

                '15Adjustable/Is the part "Adjustable"? (part should NOT be adjustable) *
                comObj.adjustable15 = If(xlsSheet1.Rows(20).Cells(i).value = Nothing, er, xlsSheet1.Rows(20).Cells(i).value.ToString())
                ...
                'mannual Questions
                '37 RefPartOccurrenceProp/reference parts?(higher level, report To PL, physical properties, all Set To "NO") * (Use occurrence property macro to recorrect setting)
                comObj.C_RefPartOccurrenceProp40 = If(xlsSheet1.Rows(21).Cells(i).value = Nothing, er, xlsSheet1.Rows(21).Cells(i).value.ToString())

                '38 AssemblyFeatures/Are the assembly features minimized? *
                comObj.C_AssemblyFeatures41 = If(xlsSheet1.Rows(23).Cells(i).value = Nothing, er, xlsSheet1.Rows(23).Cells(i).value.ToString())

                '39ModelPreview/Is the Preview of the model saved in conformance to the GEE? (On button select,
                comObj.C_ModelPreview42 = If(xlsSheet1.Rows(24).Cells(i).value = Nothing, er, xlsSheet1.Rows(24).Cells(i).value.ToString())

                '40PartConstraint/Record number of parts that are NOT fully constrained. **Record "0" if all parts are constrained correctly. (All parts locked and constraints convey design intent - i.e incorrect to "ground" parts when actual intent is for mating surfaces) *
                comObj.C_PartConstraint43 = If(xlsSheet1.Rows(25).Cells(i).value = Nothing, er, xlsSheet1.Rows(25).Cells(i).value.ToString())
                ...
                '39 lastModifiedDate
                comObj.lastModifiedDate39 = If(xlsSheet1.Rows(19).Cells(i).value = Nothing, er, xlsSheet1.Rows(19).Cells(i).value.ToString())
...
#End Region
            ...
            For i = 4 To ColCnt
                ...
                '6document number/Is the Document Number correct? (BEC designed parts must have proper doc number.Hardware And other exceptions are N/A) *
                comObj.documentNumber6 = If(xlsSheet1.Rows(10).Cells(i).value = Nothing, er, xlsSheet1.Rows(10).Cells(i).value.ToString())

                '7authorNameIs the "Author" field populated with the full name? (DGS for DGSTS models regardless of individual name) 
                comObj.AuthorName7 = If(xlsSheet1.Rows(7).Cells(i).value = Nothing, er, xlsSheet1.Rows(7).Cells(i).value.ToString())
                ...
                '15Adjustable/Is the part "Adjustable"? (part should NOT be adjustable) *
                comObj.adjustable15 = If(xlsSheet1.Rows(19).Cells(i).value = Nothing, er, xlsSheet1.Rows(19).Cells(i).value.ToString())

                '16Mat'l spec/Is the mat'l spec field populated? (indicated by RED in metadata audit file)
                comObj.Matl_spec16 = If(xlsSheet1.Rows(14).Cells(i).value = Nothing, er, xlsSheet1.Rows(14).Cells(i).value.ToString)

                '17MaterialUsed/Is the material used field populated? *
                comObj.MaterialUsed17 = If(xlsSheet1.Rows(15).Cells(i).value = Nothing, er, xlsSheet1.Rows(15).Cells(i).value.ToString)

                '18RemovedUnusedFeatures/Have any suppressed (unused) features been removed from the model Pathfinder? *
                comObj.RemovedUnusedFeatures18 = If(xlsSheet1.Rows(18).Cells(i).value = Nothing, er, xlsSheet1.Rows(18).Cells(i).value.ToString)
                ...
                '42MassAndDensity_Update/Is the density defined and the mass up to date? * (macro will auto-update if file not read only)
                comObj.C_MassAndDensity_Update45 = If(xlsSheet1.Rows(23).Cells(i).value = Nothing, er, xlsSheet1.Rows(23).Cells(i).value.ToString)
                ...
                '39 lastModifiedDate
                comObj.lastModifiedDate39 = If(xlsSheet1.Rows(21).Cells(i).value = Nothing, er, xlsSheet1.Rows(21).Cells(i).value.ToString())
                '------------------------------------------------------------------
                row1Data(comObj)
...
#End Region
            ...
            For i = 4 To ColCnt
                ...
                '19ASTMminimum/Is the bend radius of the part equal to or above the ASTM minimum? *
                comObj.ASTMminimum19 = If(xlsSheet1.Rows(16).Cells(i).value = Nothing, er, xlsSheet1.Rows(16).Cells(i).value.ToString)

                '20FlatPattern/. Is the flat pattern turned ON? (Applicable only to parts needing dxf or cut length) *
                comObj.FlatPattern20 = If(xlsSheet1.Rows(17).Cells(i).value = Nothing, er, xlsSheet1.Rows(17).Cells(i).value.ToString)

                '21HoleToolUse/Did I use the hole tool for ALL holes requiring fasteners? (include clearance holes forhardware, tapped holes, And slots) *
                comObj.HoleToolUse21 = If(xlsSheet1.Rows(18).Cells(i).value = Nothing, er, xlsSheet1.Rows(18).Cells(i).value.ToString)
                ...
                'a+p
                '39ModelPreview/Is the Preview of the model saved in conformance to the GEE? (On button select,
                comObj.C_ModelPreview42 = If(xlsSheet1.Rows(26).Cells(i).value = Nothing, er, xlsSheet1.Rows(26).Cells(i).value.ToString)
                ...
                'p+sheetmetal
                '41Material_Spec_Field/Is the material spec field filled out with the proper designation? (Modification,Purchased, Assembly, etc.) *
                comObj.C_Material_Spec_Field44 = If(xlsSheet1.Rows(23).Cells(i).value = Nothing, er, xlsSheet1.Rows(23).Cells(i).value.ToString)
                ...
                '43Bend_Radius_Update/Did I ensure the bend radius was producible? (Evaluate based on NPM0001C1 and/or consult ProdE/ManE) *
                comObj.C_Bend_Radius_Update46 = If(xlsSheet1.Rows(24).Cells(i).value = Nothing, er, xlsSheet1.Rows(24).Cells(i).value.ToString)

                '44 FlatPattern_Update/Is the flat pattern up to date? *
                comObj.C_FlatPattern_Update47 = If(xlsSheet1.Rows(25).Cells(i).value = Nothing, er, xlsSheet1.Rows(25).Cells(i).value.ToString)
...
#End Region
            ...
            For i = 4 To ColCnt
                ...
                '6document number/Is the "Document Number" field populated with the correct part number? (This should MATCH the M2M Item Master Part Number field) *
                comObj.documentNumber6 = If(xlsSheet1.Rows(14).Cells(i).value = Nothing, er, xlsSheet1.Rows(14).Cells(i).value.ToString())

                '7authorNameIs the "Author" field populated with the full name? (DGS for DGSTS models regardless of individual name) 
                comObj.AuthorName7 = If(xlsSheet1.Rows(13).Cells(i).value = Nothing, er, xlsSheet1.Rows(13).Cells(i).value.ToString())

                '8dashinunusedfield/Do all technically unused properties have a "dash" populated? 
                comObj.DashInUnusedFiled8 = If(xlsSheet1.Rows(18).Cells(i).value = Nothing, er, xlsSheet1.Rows(18).Cells(i).value.ToString())

                '9 m2mdescription/Does the Model Title MATCH the M2M Item Master (INV) Description Field?
                comObj.m2mDiscriptionMatch9 = If(xlsSheet1.Rows(12).Cells(i).value = Nothing, er, xlsSheet1.Rows(12).Cells(i).value.ToString())

                '10uomsmatch/Do all UOMs match M2M? (i.e. EA, sqft, in, etc.)
                comObj.uomsMatch10 = If(xlsSheet1.Rows(19).Cells(i).value = Nothing, er, xlsSheet1.Rows(19).Cells(i).value.ToString)
                ...
                '15Adjustable/Is the part "Adjustable"? (part should NOT be adjustable) *
                comObj.adjustable15 = If(xlsSheet1.Rows(25).Cells(i).value = Nothing, er, xlsSheet1.Rows(25).Cells(i).value.ToString())

                '16Mat'l spec/Is the "Mat'l Spec" field populated? (PURCHASED for library components) *
                comObj.Matl_spec16 = If(xlsSheet1.Rows(10).Cells(i).value = Nothing, er, xlsSheet1.Rows(10).Cells(i).value.ToString)

                '17MaterialUsed/Is the "Material Used" field populated? (PURCHASED for library components) *
                comObj.MaterialUsed17 = If(xlsSheet1.Rows(11).Cells(i).value = Nothing, er, xlsSheet1.Rows(11).Cells(i).value.ToString)

                '18RemovedUnusedFeatures/Have any suppressed (unused) features been removed from the model Pathfinder? *
                comObj.RemovedUnusedFeatures18 = If(xlsSheet1.Rows(32).Cells(i).value = Nothing, er, xlsSheet1.Rows(32).Cells(i).value.ToString)
                ...
                '21HoleToolUse/Did I use the hole tool for ALL holes requiring fasteners? (include clearance holes forhardware, tapped holes, And slots) *
                comObj.HoleToolUse21 = If(xlsSheet1.Rows(21).Cells(i).value = Nothing, er, xlsSheet1.Rows(21).Cells(i).value.ToString)

                '22virtualThread/Virtual thread applied for Fasteners?
                comObj.VirtualThread22 = If(xlsSheet1.Rows(7).Cells(i).value = Nothing, er, xlsSheet1.Rows(7).Cells(i).value.ToString)

                '23defined feature/Are ALL features fully defined in the model Pathfinder? * (Sketch fully constraint?)
                comObj.defineFeatures23 = If(xlsSheet1.Rows(8).Cells(i).value = Nothing, er, xlsSheet1.Rows(8).Cells(i).value.ToString)

                '24Supperessed Feature
                comObj.suppressedFeature24 = If(xlsSheet1.Rows(9).Cells(i).value = Nothing, er, xlsSheet1.Rows(9).Cells(i).value.ToString)

                '--------------------------------
                '25Vendorpartnumber
                comObj.vendorPartNumber25 = If(xlsSheet1.Rows(15).Cells(i).value = Nothing, er, xlsSheet1.Rows(15).Cells(i).value.ToString)
                '--------------------------------

                '26Hardwareparts
                comObj.HardwareParts26 = If(xlsSheet1.Rows(17).Cells(i).value = Nothing, er, xlsSheet1.Rows(17).Cells(i).value.ToString)

                '27m2mSourceMarked
                comObj.m2mSourceMarked27 = If(xlsSheet1.Rows(20).Cells(i).value = Nothing, er, xlsSheet1.Rows(20).Cells(i).value.ToString)

                '28SEStatus
                comObj.SEstatus28 = If(xlsSheet1.Rows(26).Cells(i).value = Nothing, er, xlsSheet1.Rows(26).Cells(i).value.ToString)
                ...
                'a+p
                '39ModelPreview/Is the Preview of the model saved in conformance to the GEE? (On button select,
                comObj.C_ModelPreview42 = If(xlsSheet1.Rows(38).Cells(i).value = Nothing, er, xlsSheet1.Rows(38).Cells(i).value.ToString)
                ...
                'MTC Baseline
                '45 HardwareInstancesAndStackups/Are all instances of hardware consistent and have logical stackups? (Examples: 1.a SS nut applied to a zinc coated bolt having a copper washer; 2. Nine(9) instances of grade 8 and one(1) instance of grade 5 within a mounting configuration) *
                comObj.C_HardwareInstancesAndStackups48 = If(xlsSheet1.Rows(29).Cells(i).value = Nothing, er, xlsSheet1.Rows(29).Cells(i).value.ToString)

                '46 DimensionalGeometry_Match_For_ALL/Has the dimensional geometry of the model been verified to match the vendor supplied data? (This is to include use of the hole tool for ALL features requiring fasteners) *
                comObj.C_DimensionalGeometry_Match_For_ALL49 = If(xlsSheet1.Rows(30).Cells(i).value = Nothing, er, xlsSheet1.Rows(30).Cells(i).value.ToString)

                '47 DimensionalGeometry_Match/Has the dimensional geometry of the model been verified to match the vendor supplied data? *
                comObj.C_DimensionalGeometry_Match50 = If(xlsSheet1.Rows(31).Cells(i).value = Nothing, er, xlsSheet1.Rows(31).Cells(i).value.ToString)

                '48 suppressedUnusedFeatures/ Have any suppressed (unused) features been removed from the model Pathfinder? *
                comObj.C_suppressedUnusedFeatures51 = If(xlsSheet1.Rows(32).Cells(i).value = Nothing, er, xlsSheet1.Rows(32).Cells(i).value.ToString)

                '49 ChildComponentsFullyConstrained/Are all child components fully constrained? (Ground constraints should be minimized for BEC generated models, but are acceptable for Vendor supplied assemblies) *
                comObj.C_ChildComponentsFullyConstrained52 = If(xlsSheet1.Rows(33).Cells(i).value = Nothing, er, xlsSheet1.Rows(33).Cells(i).value.ToString)

                '50 SimplifiedAssemblyModel/Is the assembly model simplified in design? (unnecessary details or "internal" parts should be removed if possible) *
                comObj.C_SimplifiedAssemblyModel53 = If(xlsSheet1.Rows(34).Cells(i).value = Nothing, er, xlsSheet1.Rows(34).Cells(i).value.ToString)

                '51 ChildPartOccurrenceProperties/Have the required occurrence properties for ALL child components been set to NO? (Reports/Parts List, Physical Properties, and Interference Analysis should all be set to NO) *
                comObj.C_ChildPartOccurrenceProperties54 = If(xlsSheet1.Rows(35).Cells(i).value = Nothing, er, xlsSheet1.Rows(35).Cells(i).value.ToString)

                '52 VendorMaterialData/Is the appropriate material applied to the model per Vendor supplied data? (If the component contains multiple material PURCHASED should be utilized) *
                comObj.C_VendorMaterialData55 = If(xlsSheet1.Rows(36).Cells(i).value = Nothing, er, xlsSheet1.Rows(36).Cells(i).value.ToString)

                '53 BaselineMassAndDensityUpdate/Is the density defined and the mass up to date? ("Update on file save" should be unchecked. When a Vendor weight is known "User-defined properties" should be checked and the mass applied) * (Manual check -if read-only)
                comObj.C_BaselineMassAndDensityUpdate56 = If(xlsSheet1.Rows(37).Cells(i).value = Nothing, er, xlsSheet1.Rows(37).Cells(i).value.ToString)

                '54 PMI_Instruction/Is PMI included for any other further or special situations? (kit identification, missing vendor information, etc.) *
                comObj.C_PMI_Instruction57 = If(xlsSheet1.Rows(39).Cells(i).value = Nothing, er, xlsSheet1.Rows(39).Cells(i).value.ToString)

                '55 BinderData/Does the model have a Binder attached and is properly filled out? (A hyperlink to the Vendor supplied data sheet located on the VAULT should be included) *
                comObj.C_BinderData58 = If(xlsSheet1.Rows(40).Cells(i).value = Nothing, er, xlsSheet1.Rows(40).Cells(i).value.ToString)

                '56 TerminalAssigned/Have the required terminals been assigned to the part model? *
                comObj.C_TerminalAssigned59 = If(xlsSheet1.Rows(41).Cells(i).value = Nothing, er, xlsSheet1.Rows(41).Cells(i).value.ToString)

                '57 ReferencePlanes/ Has the model been constructed on the correct reference planes per GEE0008? *
                comObj.C_ReferencePlanes60 = If(xlsSheet1.Rows(42).Cells(i).value = Nothing, er, xlsSheet1.Rows(42).Cells(i).value.ToString)
                ...
                comObj.date38 = Date.Now()
                '39 lastModifiedDate
                comObj.lastModifiedDate39 = If(xlsSheet1.Rows(28).Cells(i).value = Nothing, er, xlsSheet1.Rows(28).Cells(i).value.ToString())
                '----------------------------------------------------------------------------------------------------------
...
#End Region
            ...
            For i = 4 To ColCnt
                ...
                comObj.date38 = Date.Now()
                '39 lastModifiedDate
                comObj.lastModifiedDate39 = If(xlsSheet1.Rows(13).Cells(i).value = Nothing, er, xlsSheet1.Rows(13).Cells(i).value.ToString())
                '----------------------------------------------------------------------------------------------------------
